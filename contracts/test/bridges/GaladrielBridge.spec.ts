...
describe("GaladrielBridge", function () {
  let galadrielBridge: contracts.bridges.GaladrielBridge;
  let admin: any;
  let user: any;
  ...
  it("should allow admin to withdraw user balance", async function () {
    ...
    await galadrielBridge.connect(admin).mintOnChain(user.address, mintAmount);
  ...
  it("should revert if withdraw amount exceeds user's minted tokens", async function () {
    const mintAmount = ethers.parseEther("5");
    const withdrawAmount = ethers.parseEther("6"); // Exceeds user's minted tokens
